...
const ThemeToggle = ({ compact = false }) => {
  ...
  if (compact) {
    return (
      <button
        className="flex cursor-pointer items-center justify-center overflow-hidden rounded-lg bg-transparent text-primary text-sm font-medium leading-normal relative transition-all duration-200"
        onClick={toggleTheme}
        disabled={isTransitioning}
        aria-label={theme === 'dark' ? t.switchingToLightMode : t.switchingToDarkMode}
      >
        {theme === 'dark' ? (
